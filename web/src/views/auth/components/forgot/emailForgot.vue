...
import { emailForgot, sendForgotEmail } from '@/api'
...
const registerRules = {
  ...sendForgotEmailRules,
  emailVCode: {
    required: true,
    message: '请输入邮件验证码',
  },
}
const formRules = ref(sendForgotEmailRules)
...
async function handleClickRegister() {
  ...
  form.value?.validate().then(async () => {
    try {
      ...
      const response = await emailForgot(data)
...
<template>
  <NForm ref="form" :model="formData" :rules="formRules">
    ...
    <NFormItemRow path="password" label="修改密码">
