 ...
 *
 * @DDL(sql="CREATE TABLE `tb_card_detail` (   `id` bigint unsigned NOT NULL AUTO_INCREMENT,   `member_id` int unsigned NOT NULL COMMENT '用户ID',   `card_id` bigint unsigned NOT NULL COMMENT '卡ID',   `operation_type` tinyint unsigned NOT NULL COMMENT '操作类型',   `business_type` tinyint unsigned NOT NULL COMMENT '业务类型',   `business_id` bigint unsigned NOT NULL COMMENT '业务ID',   `change_amount` bigint NOT NULL COMMENT '变动余额',   `before_amount` bigint NOT NULL COMMENT '变动前余额',   `after_amount` bigint NOT NULL COMMENT '变动后余额',   `time` int unsigned NOT NULL COMMENT '时间',   PRIMARY KEY (`id`) USING BTREE,   KEY `business_type` (`member_id`,`business_type`,`business_id`) USING BTREE,   KEY `member_id` (`member_id`,`time` DESC) USING BTREE,   KEY `card_id` (`card_id`,`operation_type`,`business_type`,`time` DESC) USING BTREE ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci ROW_FORMAT=COMPACT COMMENT='卡明细'")
 *
...
abstract class CardDetailBase extends Model
{
     ...
     * card_id.
     *
     * @Column(name="card_id", type="bigint", length=0, accuracy=0, nullable=false, default="", isPrimaryKey=false, primaryKeyIndex=-1, isAutoIncrement=false, unsigned=true, virtual=false)
     */
     ...
     * before_amount.
     *
     * @Column(name="before_amount", type="bigint", length=0, accuracy=0, nullable=false, default="", isPrimaryKey=false, primaryKeyIndex=-1, isAutoIncrement=false, unsigned=false, virtual=false)
     */
     ...
     * after_amount.
     *
     * @Column(name="after_amount", type="bigint", length=0, accuracy=0, nullable=false, default="", isPrimaryKey=false, primaryKeyIndex=-1, isAutoIncrement=false, unsigned=false, virtual=false)
     */
